 ...
 */
    	...
    	for (int i = 1; i <= joins.size(); i++) 
    	{
    		...
    		Set<Set<LogicalJoinNode>> all_len_i_subsets = this.enumerateSubsets(joins, i);
    		for (Set<LogicalJoinNode> s : all_len_i_subsets) 
    		{
    			...
    			for (LogicalJoinNode s1 : s) 
    			{
    				...
    				CostCard plan = this.computeCostAndCardOfSubplan(stats, filterSelectivities, s1, s, best.cost, optjoin);
    				if (plan != null) 
    				{
    	...
    	if (explain == true) 
    		this.printJoins(joins, optjoin, stats, filterSelectivities);
